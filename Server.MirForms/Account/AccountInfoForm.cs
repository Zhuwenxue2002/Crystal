...
namespace Server
{
    public partial class AccountInfoForm : Form
    {
        ...
        {
            if (MessageBox.Show("确定要封禁选中的账号吗？", "封禁所选账号", MessageBoxButtons.YesNoCancel) != DialogResult.Yes) return;
        ...
        private void WipeCharButton_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("服务器运行中无法清空角色。", "提示",
                MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
                return;
            }

            if (MessageBox.Show("确定要清空数据库中的所有角色吗？", "提示",
        ...
        private void ClearStoragePasswordButton_Click(object sender, EventArgs e)
        {
            ...
            if (MessageBox.Show("确定要清除所选账号的仓库密码吗？", "提示",
                MessageBoxButtons.YesNo, MessageBoxIcon.Asterisk) != DialogResult.Yes)
                return;
        ...
        private void Delaccbtn_Click(object sender, EventArgs e)
        {
            if (AccountInfoListView.SelectedItems.Count > 0)
            {
                ...
                DialogResult result = MessageBox.Show("确定要删除该账号吗？", "确认", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
