...
namespace Server
{
    public partial class BalanceConfigForm : Form
    {
        ...
        {
            if (MessageBox.Show("确定要删除最后一个索引吗？", "删除？", MessageBoxButtons.YesNo) != DialogResult.Yes) return;
            RandomItemStatsChanged = true;
        ...
        private void classGridView_CellValidating(object sender, DataGridViewCellValidatingEventArgs e)
        {
            ...
            if (e.ColumnIndex == 2 || e.ColumnIndex == 5)
            {
                ...
                {
                    e.Cancel = true;
                    classGridView.Rows[e.RowIndex].ErrorText = "该值必须为整数";
                }
            }
            else if (e.ColumnIndex == 3 || e.ColumnIndex == 4)
            {
                ...
                {
                    e.Cancel = true;
                    classGridView.Rows[e.RowIndex].ErrorText = "该值必须为小数";
                }
            }
        }
        ...
        private void classCapGridView_CellValidating(object sender, DataGridViewCellValidatingEventArgs e)
        {
            ...
            if (e.ColumnIndex == 1)
            {
                ...
                {
                    e.Cancel = true;
                    classCapGridView.Rows[e.RowIndex].ErrorText = "该值必须为整数";
                }
            }
        }
        ...
        private void UpdateBaseStatData()
        {
            ...
            foreach (DataGridViewRow row in classGridView.Rows)
            {
                ...
                {
                    MessageBox.Show(string.Format($"属性“{type}”重复出现，将不会保存。"));
                    continue;
                }
        ...
        private void heroGridView_CellValidating(object sender, DataGridViewCellValidatingEventArgs e)
        {
            ...
            if (e.ColumnIndex == 2 || e.ColumnIndex == 5)
            {
                ...
                {
                    e.Cancel = true;
                    heroGridView.Rows[e.RowIndex].ErrorText = "该值必须为整数";
                }
            }
            else if (e.ColumnIndex == 3 || e.ColumnIndex == 4)
            {
                ...
                {
                    e.Cancel = true;
                    heroGridView.Rows[e.RowIndex].ErrorText = "该值必须为小数";
                }
            }
        }
        ...
        private void heroCapGridView_CellValidating(object sender, DataGridViewCellValidatingEventArgs e)
        {
            ...
            if (e.ColumnIndex == 1)
            {
                ...
                {
                    e.Cancel = true;
                    heroCapGridView.Rows[e.RowIndex].ErrorText = "该值必须为整数";
                }
            }
        }
