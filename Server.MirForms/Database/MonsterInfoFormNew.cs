...
namespace Server.Database
{
    public partial class MonsterInfoFormNew : Form
    {
        ...
        private void monsterInfoGridView_CellValidating(object sender, DataGridViewCellValidatingEventArgs e)
        {
            ...
            {
                e.Cancel = true;
                monsterInfoGridView.Rows[e.RowIndex].ErrorText = "该值必须为正整数";
            }

            if (col.ValueType == typeof(int) && !int.TryParse(val, out _))
            {
                e.Cancel = true;
                monsterInfoGridView.Rows[e.RowIndex].ErrorText = "该值必须为整数";
            }
            else if (col.ValueType == typeof(byte) && !byte.TryParse(val, out _))
            {
                e.Cancel = true;
                monsterInfoGridView.Rows[e.RowIndex].ErrorText = "该值必须为 byte";
            }
            else if (col.ValueType == typeof(short) && !short.TryParse(val, out _))
            {
                e.Cancel = true;
                monsterInfoGridView.Rows[e.RowIndex].ErrorText = "该值必须为 short";
            }
            else if (col.ValueType == typeof(ushort) && !ushort.TryParse(val, out _))
            {
                e.Cancel = true;
                monsterInfoGridView.Rows[e.RowIndex].ErrorText = "该值必须为 ushort";
            }
            else if (col.ValueType == typeof(long) && !long.TryParse(val, out _))
            {
                e.Cancel = true;
                monsterInfoGridView.Rows[e.RowIndex].ErrorText = "该值必须为 long";
            }
        }
                    ...
                    if (!fileError)
                    {
                        ...
                        for (int i = 1; i < rows.Length; i++)
                        {
                                ...
                                for (int j = 1; j < columns.Length; j++)
                                {
                                    ...
                                    if (dataColumn.Name == "MonsterName")
                                    {
                                        ...
                                        if (existingRow != null)
                                        {
                                            ...
                                            {
                                                throw new Exception($"已存在名为 {cells[j]} 的怪物。");
                                            }
                                        }
                            ...
                            catch (Exception ex)
                            {
                                ...
                                MessageBox.Show($"导入怪物 {cells[0]} 时出错。{ex.Message}");
                                break;
                            }
                        }
                        ...
                        if (!fileError)
                        {
                            MessageBox.Show($"已导入 {rowsEdited} 个怪物。");
                        }
                        else
                        {
