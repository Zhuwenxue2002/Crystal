...
状态：已于 2026-06-05 收口。名称引用联动汉化不并入第五阶段，转入第六阶段。

## 第六阶段：名称引用联动汉化

1. 建立物品、怪物、NPC、地图/传送点英文名到中文名的映射表。
2. 对每个名称同步处理：
   - 数据库名称字段
   - NPC 脚本引用
   - 任务键和任务正文引用
   - 掉落表、配方、交易清单、GM 清单
   - 服务端配置和事件脚本
3. 优先选择引用少、引用位置清晰、可完整同步的一组名称。
4. 每批后启动服务端，检查物品、怪物、NPC、任务和掉落加载日志。
5. 保留英文原名到中文名的映射与引用修改清单，方便回滚和查漏。
